...
drop table if exists `job_logs_${AUX_CHAIN_ID}`;
CREATE TABLE job_logs_${AUX_CHAIN_ID}
(
  ID_JOB INT
...
;
CREATE INDEX IDX_job_logs_${AUX_CHAIN_ID}_1 ON job_logs_${AUX_CHAIN_ID}(ID_JOB)
;
CREATE INDEX IDX_job_logs_${AUX_CHAIN_ID}_2 ON job_logs_${AUX_CHAIN_ID}(ERRORS, STATUS, JOBNAME)
;



drop table if exists `transformation_logs_${AUX_CHAIN_ID}`;
-- Transformation log table
--

CREATE TABLE transformation_logs_${AUX_CHAIN_ID}
(
  ID_BATCH INT
, CHANNEL_ID VARCHAR(255)
...
;
CREATE INDEX IDX_transformation_logs_${AUX_CHAIN_ID}_1 ON transformation_logs_${AUX_CHAIN_ID}(ID_BATCH)
;
...
;
  ...
  drop table if exists `dim_tokens_${AUX_CHAIN_ID}` ;
  
  CREATE TABLE `dim_tokens_${AUX_CHAIN_ID}` (
  ...
  ALTER TABLE `dim_tokens_${AUX_CHAIN_ID}` ADD UNIQUE INDEX  u_token_id (token_id);

  drop table if exists `dim_meta_names_${AUX_CHAIN_ID}`;
  
  CREATE TABLE `dim_meta_names_${AUX_CHAIN_ID}` (
  ...
  ALTER TABLE `dim_meta_names_${AUX_CHAIN_ID}` ADD UNIQUE INDEX u_meta_name_token_id (meta_name, token_id);
  ...
  ALTER TABLE `token_transfer_facts_aux_${AUX_CHAIN_ID}` ADD column total_volume_in_eth decimal(40,10) NOT NULL DEFAULT 0 AFTER total_volume;