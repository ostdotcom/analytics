const rootPrefix = "..",
    ...
    auxBlockScannerConfigFile = Constants.AUX_BLOCK_SCANNER_CONFIG_FILE,
    originBlockScannerConfigFile = Constants.ORIGIN_BLOCK_SCANNER_CONFIG_FILE,
...
class BlockScanner {
        ...
        if(oThis.chainType == blockScannerGC.auxChainType){
            oThis.blockScannerConfigFile = auxBlockScannerConfigFile;
        }else if(oThis.chainType == blockScannerGC.originChainType){
            oThis.blockScannerConfigFile = originBlockScannerConfigFile;
        }else{
        ...
        oThis.blockScanner = new OSTBlockScanner(oThis.blockScannerConfigFile);
    ...
    validateBlockScannerConfig(chainId) {
            const oThis = this;
        for (let chainConfig of oThis.blockScannerConfigFile['chains']) {
    ...
    async getTransactionHashes(blockNumber) {
        ...
        while (true) {
            ...
            data = response.data;
            if (data.transactionHashes && data.transactionHashes.length) {
