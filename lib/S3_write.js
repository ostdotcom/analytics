...
const S3Write = function (s3Config, params) {
    ...
    oThis.filePrefix = params.file_prefix;
    oThis.numberOfFile = params.number_of_files;
    oThis.separator = params.separator;
    oThis.s3BucketLink = params.s3_bucket_link;
...
S3Write.prototype.uploadFile = async function ( file_name) {
    ...
    fs.readFile(oThis.dirPath + file_name, function (err, data) {
        ...
        logger.debug('uploading file..', oThis.bucketPath + file);
        let base64data = new Buffer(data, 'binary');
        oThis.s3Client.putObject({
            Bucket: oThis.s3BucketLink,
            Key: oThis.bucketPath + file,
            Body: base64data
        }, function (err, data) {
            if (err) {
                return Promise.resolve(err);
...
S3Write.prototype.write = async function (dataArray, file_number) {
    const oThis = this
        , fileName = oThis.dirPath + oThis.filePrefix + file_number
    ;
...
S3Write.prototype.uploadFiles = async function () {
    ...
    try {
        ...
        for (let fileNumber = 0; fileNumber < files.length; fileNumber++) {
            promiseArray.push(oThis.uploadFile(oThis.s3Client, files[fileNumber]));
