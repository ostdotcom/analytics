...
S3Write.prototype.uploadFiles = async function () {
  ...
  try {
    ...
    for (let fileNumber = 0; fileNumber < files.length; fileNumber++) {
      ...
      logger.info('File upload...', oThis.dirPath + "/" + files[fileNumber]);
    }

    await Promise.all(promiseArray).catch(function (err) {
      logger.error("ERROR IN UPLOAD FILES PROMISES", err);

      return responseHelper.error({
        internal_error_identifier: 'l_s3w_uf_1',
    ...
    return Promise.resolve(responseHelper.error({
      internal_error_identifier: 'l_s3w_uf',
      api_error_identifier: 'upload_to_s3',
      debug_options: {}
    }));
  }
...
S3Write.prototype.deleteLocalDirectory = function (localDirFullFilePath) {
...
S3Write.uploadToS3 = async function (s3UploadPath, localDirFullFilePath) {
  ...
  s3Write.deleteLocalDirectory(localDirFullFilePath);
  return res;
};
