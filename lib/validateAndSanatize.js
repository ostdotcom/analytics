...
class ValidateAndSanitize{
    ...
    validateField(columnInfo, value){
        ...
        if (oThis.isPresent(value) && columnInfo[1]['max']  && parseInt(value) > columnInfo[1]['max']){
            return responseHelper.error(
                {
                    internal_error_identifier: 'm_r_b_b_vf_3',
                    api_error_identifier: fieldName + '_is_greater_than_valid',
                    debug_options: oThis.object
                }
            )
        }
    ...
    validateAndFormatData() {
        ...
        for (let column of oThis.mapping) {
            ...
            if(! r.success){
                oThis.applicationMailer.perform({subject: r.apiErrorIdentifier + "Validation failed" , body: {err: r}});
                if (false && (constants.ENVIRONMENT == "staging" || constants.ENVIRONMENT == "development")){
