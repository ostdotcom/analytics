...
class redshift {
     ...
     *
     */
    async parameterizedQuery(commandString) {
        const oThis = this
        ;
        logger.debug('Redshift parameterizedQuery String', commandString);
        return new Promise(function (resolve, reject) {
            try {
                oThis.redshiftClient.parameterizedQuery(commandString, function (err, result) {
                    if (err) {
